...
const TABS = [
  ...
  { path: '/amis', label: 'Amis', icon: (a) => <svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={a?2.5:1.8} strokeLinecap="round"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/></svg> },
  { path: '/defis', label: 'Défis', icon: (a) => <svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={a?2.5:1.8} strokeLinecap="round"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg> },
    ...
    <>
      ...
      {location.pathname !== '/chat' && location.pathname !== '/poster' && (
        <button onClick={() => navigate('/poster')} style={{
          ...
          background: 'var(--am)', border: 'none', cursor: 'pointer',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          zIndex: 101, animation: 'pulse-fab 3s infinite',
        }}>
          ...
          {TABS.map(tab => {
            ...
            return (
              <button key={tab.path} onClick={() => navigate(tab.path)} style={{
                flexShrink: 0, minWidth: 64, height: '100%',
                display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
                gap: 3, color: active ? 'var(--am)' : 'var(--tx3)',
                background: 'none', border: 'none', cursor: 'pointer', transition: 'color .15s',
                position: 'relative', padding: '0 4px',
              }}>
                {active && <div style={{ position: 'absolute', top: 0, left: '50%', transform: 'translateX(-50%)', width: 24, height: 2, borderRadius: 1, background: 'var(--am)' }} />}
                {tab.icon(active)}
                <span style={{ fontSize: 9, fontFamily: 'DM Sans,sans-serif', letterSpacing: '.03em', fontWeight: active ? 500 : 400 }}>{tab.label}</span>
              </button>
            )
          })}
        </div>
